                ...
                try:
                    ...
                    if error_code in ['ThrottlingException', 'TooManyRequestsException']:
                        if attempt < max_retries:
                            # Exponential backoff với jitter - longer delays cho Bedrock
...
# Global rate limiter instance - More reasonable for inference profiles
bedrock_rate_limiter = BedrockRateLimiter(
    requests_per_minute=5,  # 5 requests per minute - reasonable for inference profiles  
    max_retries=3,
